...
<div class="container">
    <header class="header">
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <select id="globalFY" onchange="updateLastReqInfo()" style="padding: 6px 12px; font-size: 16px; font-weight: bold; border-radius: 6px; color: var(--deep-blue); background-color: #e3f2fd; border: 2px solid #90caf9; cursor: pointer;">
            </select>
        </div>
...
<nav class="tabs">
    ...
    <main id="itemManage" class="tab-content active">
        <div class="sub-tabs">
            ...
            <button class="sub-tab-button" onclick="openSubTab(event, 'stockAudit', 'itemManage')">STOCK AUDIT (EXCEL)</button>
        </div>
<div id="manageMiscItems" class="sub-tab-content">
    ...
    <div id="manageMiscPaginationControls" class="pagination-controls"></div>
        ...
        <div id="itemList" class="sub-tab-content">
            ...
            <div id="paginationControls" class="pagination-controls"></div>
             ...
             <button class="btn btn-secondary mt-20" onclick="exportItemsForUpdate()" style="margin-bottom: 15px;">⬇️ Download Current Item List (Excel)</button>
    ...
    <tr>
        ...
        <div id="stockAudit" class="sub-tab-content">
            <h3>Year-End Stock Audit (Google Sheet)</h3>
            <div style="background: #e3f2fd; padding: 15px; border-radius: 6px; border: 1px solid #90caf9; margin-bottom: 20px;">
                <p style="margin:0; color: #1565c0;">
                    <strong>ধাপ ১:</strong> নিচের 'Prepare Audit Sheet' বাটনে ক্লিক করুন। এটি আপনার গুগল শিটে অটোমেটিক <b>Audit Report</b> নামের একটি নতুন ট্যাব তৈরি করে দেবে।<br>
                    <strong>ধাপ ২:</strong> গুগল শিটটি ওপেন করে আপনার গোনা স্টকের সংখ্যাগুলো সেখানে বসান।<br>
                    <strong>ধাপ ৩:</strong> এরপর এই সফটওয়্যারটি রিলোড (F5) করে এসে নিচের 'Sync Audit' বাটনে ক্লিক করুন। নতুন বছরে আপনার স্টক এখান থেকেই শুরু হবে!
                </p>         
            <button class="btn btn-secondary" onclick="prepareAuditSheet()" style="margin-right: 10px; margin-bottom: 10px;">📄 1. Prepare Audit Sheet (Google Sheet)</button>
            <button class="btn btn-success" onclick="syncAuditFromSheet()" style="margin-bottom: 10px;">🔄 2. Sync & Update Stock from Google Sheet</button>
            
            <div id="auditResult" class="mt-20" style="padding: 15px; background: #fff; border-radius: 8px; display: none;"></div>
        </div>
            
            <div class="form-group" style="max-width: 500px;">
                <label>Select Audit Excel File (.xlsx)</label>
                <input type="file" id="auditExcelInput" accept=".xlsx, .xls" style="padding: 10px; border: 1px solid #ccc; background: white;">
                <p style="font-size: 13px; margin-top: 8px; color: var(--text-red);">
                    <strong>এক্সেল ফাইলে অবশ্যই এই দুটি কলাম থাকতে হবে:</strong> 'Item Name' এবং 'Counted Stock'
                </p>
            </div>
             <button class="btn btn-success mt-20" onclick="processAuditFromExcel()">📊 Process Audit & Adjust Stock</button>
...
// ==========================================
// --- GLOBAL HELPER FOR ITEM CODE SORTING ---
// ==========================================
function sortStoreItemsByCode(a, b) {
    const codeA = (a.code || "").toString().trim().toUpperCase();
    const codeB = (b.code || "").toString().trim().toUpperCase();
    
    // কোড অনুযায়ী A-Z সাজাবে
    if (codeA < codeB) return -1;
    if (codeA > codeB) return 1;
    
    // যদি কোড একই হয়, তবে নাম অনুযায়ী সাজাবে
...
function populateItemDatalist() {
    const itemListData = document.getElementById('itemListData');
    if (itemListData) {
        const sortedItems = [...appData.items].sort(sortStoreItemsByCode);
        const optionsHtml = sortedItems.map(item => `<option value="${item.name}"></option>`).join('');
        itemListData.innerHTML = optionsHtml;
    }
}
...
function populateAllDropdowns() {
    ...
    // 🟢 Item Ledger Report-এর ড্রপডাউনটিকে Item Code অনুযায়ী সাজানো হচ্ছে
    const ledgerItemSelect = document.getElementById('ledgerItemSelect');
    if (ledgerItemSelect) {
        const sortedItems = [...appData.items].sort(sortStoreItemsByCode);
        
        ledgerItemSelect.innerHTML = '<option value="">-- Select Store Item --</option>';
        sortedItems.forEach(item => {
            const displayText = item.code ? `${item.code} - ${item.name}` : item.name;
            ledgerItemSelect.innerHTML += `<option value="${item.id}">${displayText}</option>`;
        });
    }
} // 🔴 এই শেষের ব্র্যাকেটটাই আগেরবার মুছে গিয়েছিল!
...
function updateMiscItemStock(itemName, qtyChange, unit = appData.miscItemDefaults.unit) {
    ...
    if (miscItem) {
        // FIX: Force math addition instead of string concatenation
        miscItem.stock = Number(miscItem.stock) + Number(qtyChange);
    } else if (qtyChange > 0) {
        appData.miscItems.push({
            id: getNextId('misc'),
            name: itemName,
            stock: Number(qtyChange), // FIX: Force Number
            unit: unit 
        });
    }
}
...
function toggleRequisitionItemType(prefix) {
    ...
    let sourceData = [];
    if (itemType === 'store') {
        sourceData = appData.items.filter(i => i.stock > 0).sort(sortStoreItemsByCode);
    } else {
        sourceData = appData.miscItems.filter(i => i.stock > 0).sort((a, b) => a.name.localeCompare(b.name));
    }
...
function renderAll() {
    populateFinancialYearDropdown(); // <--- এই লাইনটি একদম প্রথমে যোগ করুন
    ...
    // সর্বশেষ আপডেট হওয়া ইনফো দেখানোর জন্য
...
function populateAllDropdowns() {
    ...
    const ledgerItemSelect = document.getElementById('ledgerItemSelect');
    if (ledgerItemSelect) {
        // আইটেমগুলোকে Item Code অনুযায়ী সাজানো হচ্ছে
        const sortedItems = [...appData.items].sort(sortStoreItemsByCode);
        
        // ড্রপডাউন লিস্ট ক্লিয়ার করে নতুন করে তৈরি করা
        ledgerItemSelect.innerHTML = '<option value="">-- Select Store Item --</option>';
        sortedItems.forEach(item => {
            // সেফটি চেক: যদি কোড থাকে তবেই "Code - Name" দেখাবে
            const displayText = item.code ? `${item.code} - ${item.name}` : item.name;
            ledgerItemSelect.innerHTML += `<option value="${item.id}">${displayText}</option>`;
        });
    }
...
function populateMiscDatalist() {
    ...
    function filterItemList() {
        ...
        // 2. Filter if there is a search term
        if (searchTerm) {
        ...
        // 3. SORT BY ITEM CODE (Alphanumeric order: A0001, B0001, B0002)
        filteredItems.sort((a, b) => {
            const codeA = (a.code || "").toString().toUpperCase();
            const codeB = (b.code || "").toString().toUpperCase();
            
            // This ensures strict alphabetical/numerical sorting
...
// --- EXPORT ITEM LIST FOR BULK UPDATE ---
function exportItemsForUpdate() {
    if (appData.items.length === 0) {
        alert("No items found to export.");
        return;
    }

    // এক্সেলের জন্য ডেটা সাজানো হচ্ছে
    const data = appData.items.map(item => ({
        'Item Code': item.code,
        'Item Name': item.name,
        'Stock Page No.': item.stockPage,
        'Purch. Page No.': item.purchPage,
        'Unit Measure': item.unit,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Items To Update");

    // ফাইলটি ডাউনলোড করা
    const fyString = document.getElementById('globalFY') ? document.getElementById('globalFY').value : 'New_FY';
    XLSX.writeFile(workbook, `Item_Master_Page_Update_${fyString}.xlsx`);
}
...
function saveChallan() {
    ...
    newChallan.items.forEach(challanItem => {
        if (challanItem.isMisc) {
            ...
            // FIX: Match IDs by casting both to Strings
            const itemInDb = appData.items.find(i => String(i.id) === String(challanItem.itemId));
            if (itemInDb) {
                // FIX: Force math addition
                itemInDb.stock = Number(itemInDb.stock) + Number(challanItem.qty);
            }
        }
    });
...
function saveDirectInvoice() {
    ...
    newInvoice.items.forEach(invoiceItem => {
        if (invoiceItem.isMisc) {
            ...
            // FIX: Match IDs by casting both to Strings
            const itemInDb = appData.items.find(i => String(i.id) === String(invoiceItem.itemId));
            if (itemInDb) {
                // FIX: Force math addition
                itemInDb.stock = Number(itemInDb.stock) + Number(invoiceItem.qty);
            }
        }
    });
...
function filterInvoiceList(fromPagination = false) {
    // 1. Only reset to Page 1 if a filter was changed (NOT a page click)
    if (!fromPagination) {
...
function saveRequisition() {
    ...
    try {
       ...
       // 3. Strict Duplicate Check (Financial Year Based for R- and RD-)
       if (appData && appData.requisitions) {
            const currentFY = getFinancialYear(reqDate); // যেই তারিখ দেওয়া হয়েছে তার FY বের করা
            
            const isDuplicate = appData.requisitions.some(req => 
                req && req.slipNo && req.date &&
                String(req.slipNo).toLowerCase() === reqSlipNoRaw.toLowerCase() &&
                getFinancialYear(req.date) === currentFY
            );

            if (isDuplicate) {
                alert(`⚠️ Error: Requisition Slip Number "${reqSlipNoRaw}" already exists in Financial Year ${currentFY}!\n\nএকই আর্থিক বছরে (Financial Year) একই স্লিপ নম্বর (R- হোক বা RD-) দ্বিতীয়বার ব্যবহার করা যাবে না।`);
                return; 
            }
        } else {
      ...
      newRequisition.items.forEach(reqItem => {
            if (reqItem.isMisc) {
                ...
                // FIX: Match IDs by casting both to Strings
                const itemInDb = appData.items.find(i => String(i.id) === String(reqItem.itemId));
                if (itemInDb) {
                    // FIX: Force math subtraction
                    itemInDb.stock = Number(itemInDb.stock) - Number(reqItem.qty);
                } else {
...
function populateRequisitionFilters() {
    ...
    if (storeItemSelect) {
        const sortedItems = [...appData.items].sort(sortStoreItemsByCode);
        storeItemSelect.innerHTML = '<option value="">All Store Items</option>'; 
        sortedItems.forEach(item => {
    ...
    if (miscItemSelect) {
        ...
        appData.requisitions.forEach(req => {
            req.items.forEach(item => { if (item.isMisc) miscItemNames.add(item.name); });
        });
...
function viewCurrentStockValueReport() {
    ...
    function renderStoreItems(itemsToRender = appData.items) {
    const listBody = document.getElementById('storeItemListBody');
    if (!listBody) return;
    
    // 🟢 আইটেমগুলোকে Item Code অনুযায়ী সাজানো হচ্ছে
    const sortedItems = [...itemsToRender].sort(sortStoreItemsByCode);
        ...
        const row = `
            <tr>
                ...
                <td class="action-buttons">
                    <button class="btn-edit" onclick="editStoreItem(${item.id})">Edit</button>
...
function previewItemLedgerReport() {
    ...
    // --- Helper Function: Get Challan Date instead of Invoice Date ---
    const getEffectiveDate = (tx) => {
        if (tx.vendor && tx.challans && tx.challans.length > 0 && tx.challans[0].date) {
            return tx.challans[0].date; // সর্বদা Challan Date ব্যবহার করবে
        }
        return tx.date;
    };
    ...
    txs.filter(tx => {
        const eDate = new Date(getEffectiveDate(tx));
        return eDate >= new Date(startDate) && eDate <= new Date(today);
    }).forEach(tx => {
    ...
    txs.filter(tx => {
        ...
        tx.items.forEach(txItem => {
            if (txItem.itemId === itemId && !txItem.isMisc) {
                const eDate = getEffectiveDate(tx);
                ...
                if (tx.vendor) {
                    ...
                    if (tx.vendor === "AUDIT_ADJUSTMENT") {
                        typeLabel = 'AUDIT COUNT';
                        refNo = tx.number; // এটি অটোমেটিক "2025-2026 Audit Counted Stock" দেখাবে
                        subText = '<span style="color: #2980b9; font-weight: bold;">NEW YEAR STOCK</span>';
                    }
                ...
                if (tx.department) {
                    ...
                    if (tx.department === "FY Audit Shortage (Write-off)") {
                        typeLabel = 'AUDIT (SHORTAGE)';
                        subText = '<span style="color: #c0392b; font-weight: bold;">STOCK AFTER AUDIT</span>';
                    }
    ...
    transactions.sort((a, b) => {
        ...
        const isInputA = (a.type === 'PURCHASE' || a.type === 'CHALLAN' || a.type === 'AUDIT (SURPLUS)');
        const isInputB = (b.type === 'PURCHASE' || b.type === 'CHALLAN' || b.type === 'AUDIT (SURPLUS)');
    ...
    transactions.forEach(t => {
        ...
        const rowClass = (t.type === 'PURCHASE' || t.type === 'CHALLAN' || t.type === 'AUDIT (SURPLUS)') ? 'ledger-row-purchase' : 'ledger-row-issue';
...
function exportItemLedgerReport(format) {
        ...
        tx.items.forEach(txItem => {
            if (txItem.itemId === itemId && !txItem.isMisc) {
                ...
                if (tx.vendor) {
                    ...
                    if (tx.department === "AUDIT_ADJUSTMENT") {
                        typeLabel = 'AUDIT COUNT';
                        refNo = tx.slipNo; 
                        subText = '<span style="color: #2980b9; font-weight: bold;">NEW YEAR STOCK</span>';
                    }
                ...
                if (tx.department) {
                    ...
                    transactions.push({ 
                        date: eDate, type: typeLabel, 
                        ref: refText, party: tx.department,
    ...
    transactions.sort((a, b) => {
        ...
        if (isInputA && !isInputB) return -1; 
        if (!isInputA && isInputB) return 1;  
        const numA = parseInt(String(a.ref || '').replace(/[^0-9]/g, ''), 10) || 0;
        const numB = parseInt(String(b.ref || '').replace(/[^0-9]/g, ''), 10) || 0;
        return numA - numB; 
    });
    ...
    if (format === 'pdf') {
        ...
        doc.autoTable({
            ...
            didParseCell: function (data) {
                if (data.section === 'body' && (data.row.raw[1] === 'PURCHASE' || data.row.raw[1] === 'CHALLAN' || data.row.raw[1] === 'AUDIT (SURPLUS)')) {
...
function updateLastReqInfo() {
    ...
    // হেডারের ড্রপডাউন থেকে সিলেক্ট করা Financial Year নেওয়া
    const globalFYEl = document.getElementById('globalFY');
    let targetFY = globalFYEl ? globalFYEl.value : getFinancialYear(new Date().toISOString().split('T')[0]);
    ...
    appData.requisitions.forEach(req => {
        if (!req.slipNo || !req.date) return;
        
        // শুধু সিলেক্ট করা Financial Year এর ডেটা চেক করবে
        if (getFinancialYear(req.date) !== targetFY) return;

        const slipUpper = req.slipNo.toUpperCase();
        if (slipUpper.startsWith('R-')) {
            const num = parseInt(slipUpper.replace('R-', ''));
            if (!isNaN(num) && num > maxRNum) {
                maxRNum = num;
                lastStore = req;
            }
        } else if (slipUpper.startsWith('RD-')) {
            const num = parseInt(slipUpper.replace('RD-', ''));
    ...
    const fmtInfo = (req) => {
        if (!req) return "<span style='color:green;'>No entry yet (Start from 1)</span>";
        return `<b>${req.slipNo}</b> (Date: ${formatDateToDDMMYYYY(req.date)}, Dept: ${req.department})`;
    };

    displayEl.innerHTML = `
        <div style="margin-bottom: 8px; font-weight: bold; color: #d35400;">Showing Data For Financial Year: ${targetFY}</div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            <div><small>Latest Store Req (R-):</small><br>${fmtInfo(lastStore)}</div>
            <div><small>Latest Misc Req (RD-):</small><br>${fmtInfo(lastMisc)}</div>
        </div>
    `;
}

function populateFinancialYearDropdown() {
    const fySelect = document.getElementById('globalFY');
    if (!fySelect) return;
    fySelect.innerHTML = '';
    ...
    // ৫ বছরের অপশন তৈরি করা (আগের ২ বছর, বর্তমান বছর, পরের ২ বছর)
    let startYear = currentYear - 2;
    for (let i = 0; i < 5; i++) {
        const year1 = startYear + i;
        const year2 = year1 + 1;
        const fyString = `${year1}-${year2}`;
        
        const option = document.createElement('option');
        option.value = fyString;
        option.textContent = `FY: ${year1} - ${year2}`;
        fySelect.appendChild(option);
    }
    
    // বর্তমান মাস অনুযায়ী ডিফল্ট Financial Year সিলেক্ট করা (এপ্রিল থেকে শুরু)
    let activeFY = '';
    if (currentMonth >= 4) {
        activeFY = `${currentYear}-${currentYear + 1}`;
    } else {
        activeFY = `${currentYear - 1}-${currentYear}`;
    }
...
function getFinancialYear(dateString) {
    ...
    if (month >= 4) {
        return `${year}-${year + 1}`; // April to March
    } else {
        return `${year - 1}-${year}`; // Jan to March
    }
}
...
function populateFinancialYearDropdown() {
    const fySelect = document.getElementById('globalFY');
    if (!fySelect) return;
    fySelect.innerHTML = ''; // আগের সব অপশন ক্লিয়ার করে দেওয়া
...
// ==========================================
// --- GOOGLE SHEET AUDIT FUNCTIONALITY ---
// ==========================================
function prepareAuditSheet() {
    if(!confirm("এটি গুগল শিটে 'Audit Report' নামের ট্যাবটি তৈরি করবে। আপনি কি নিশ্চিত?")) return;
    
    // বর্তমান আর্থিক বছর বের করা
    const fy = document.getElementById('globalFY') ? document.getElementById('globalFY').value : '2025-2026';
    const countColName = `${fy} Counted Stock`; // ডাইনামিক হেডিং

    appData.auditReport = appData.items.map(item => ({
        'Store Item': item.name,
        'Item Code': item.code,
        'Current Stock': item.stock,
        [countColName]: '' // ইউজার ইনপুটের জন্য ফাঁকা থাকবে
    }));
    
    saveData(); // গুগলে সেভ হবে
    alert(`Google Sheet-এ 'Audit Report' ট্যাব তৈরি হয়ে গেছে! দয়া করে গুগল শিট ওপেন করে '${countColName}' কলামটিতে আপনার স্টকের সংখ্যা বসান।`);
}

// ==========================================
// --- GOOGLE SHEET AUDIT FUNCTIONALITY ---
// ==========================================
function syncAuditFromSheet() {
    try {
        // ১. গুগল শিটের ডেটা চেক করা
        if(!appData.auditReport || appData.auditReport.length === 0) {
            alert("⚠️ Audit Report Data is empty!\n\nকারণ: আপনি হয়তো গুগল শিটে ডেটা বসানোর পর এই পেজটি রিলোড (F5) করেননি।\nদয়া করে পেজটি একবার রিলোড করে আবার চেষ্টা করুন।");
            return;
        }
        ...
        // 🟢 ফাইন্যান্সিয়াল ইয়ার এবং অডিটের তারিখ (৩১ মার্চ) বের করা
        const fy = document.getElementById('globalFY') ? document.getElementById('globalFY').value : '2025-2026';
        const endYear = fy.split('-')[1]; // যেমন: 2026
        const auditDate = `${endYear}-03-31`; // সব সময় ওই বছরের ৩১ মার্চ হবে!

        // ২. প্রতিটি আইটেম চেক ও স্টক পরিবর্তন করা
        appData.auditReport.forEach(row => {
            const keys = Object.keys(row);
            
            // সঠিক কলাম খোঁজা 
            const countKey = keys.find(k => k.toLowerCase().includes('count'));
            const codeKey = keys.find(k => k.toLowerCase().includes('code'));
            const currentStockKey = keys.find(k => k.toLowerCase().trim() === 'current stock'); // 🟢 শিটে থাকা আগের স্টক
            
            if(!countKey || !codeKey || !currentStockKey) return;

            const countedStockRaw = row[countKey];
            
            // যদি গুগল শিটে কোনো সংখ্যা বসানো থাকে
            if(countedStockRaw !== '' && countedStockRaw !== null && countedStockRaw !== undefined) {
                const countedStock = parseInt(String(countedStockRaw).trim(), 10);
                const sheetFrozenStock = parseInt(String(row[currentStockKey]).trim(), 10); // 🟢 অডিট শিট তৈরির সময়কার স্টক
                
                if(!isNaN(countedStock) && !isNaN(sheetFrozenStock)) {
                    const rowCode = String(row[codeKey]).trim().toLowerCase();
                    const item = appData.items.find(i => String(i.code).trim().toLowerCase() === rowCode);
                    
                    if(item) {
                        // 🟢 শিটের পুরনো স্টকের সাথে গোনা স্টকের পার্থক্য (Shortage/Surplus) বের করা হচ্ছে
                        const diff = countedStock - sheetFrozenStock; 
                        
                        if (diff > 0) {
                            // Surplus (স্টক বেশি পাওয়া গেছে)
                            surplusItems.push({ itemId: item.id, name: item.name, qty: diff, rate: 0, gst: 0, unit: item.unit, isMisc: false, value: 0 });
                            item.stock = parseInt(item.stock || 0) + diff; // 🟢 লাইভ স্টকের সাথে শুধু পার্থক্যটা যোগ করা হলো
                            processedCount++;
                            logText += `➕ ${item.name}: ${sheetFrozenStock} ➔ ${countedStock} (Added: ${diff})\n`;
                        } else if (diff < 0) {
                            // Shortage (স্টক কম পাওয়া গেছে / Deduct হবে)
                            shortageItems.push({ itemId: item.id, name: item.name, code: item.code, qty: Math.abs(diff), unit: item.unit, isMisc: false });
                            item.stock = parseInt(item.stock || 0) + diff; // 🟢 লাইভ স্টকের সাথে পার্থক্যটা বিয়োগ করা হলো (diff মাইনাস থাকায় বিয়োগ হবে)
                            processedCount++;
                            logText += `➖ ${item.name}: ${sheetFrozenStock} ➔ ${countedStock} (Deducted: ${Math.abs(diff)})\n`;
                        }
                    }
                }
            }
        ...
        // ৩. কাজ সফল হলে সেভ করা এবং লেজার এন্ট্রি করা
        if (processedCount > 0) {
            const auditVendorDept = "AUDIT_ADJUSTMENT"; 
            if (!appData.vendors.some(v => v.name === auditVendorDept)) appData.vendors.push({ id: getNextId('vendor'), name: auditVendorDept });
            if (!appData.departments.some(d => d.name === auditVendorDept)) appData.departments.push({ id: getNextId('department'), name: auditVendorDept });

            const refString = `${fy} Audit Counted Stock`;
            
            // 🟢 ডেট হিসেবে auditDate (৩১ মার্চ) ব্যবহার করা হচ্ছে (যাতে লেজারে পারফেক্টলি বসে)
            if(surplusItems.length > 0) {
                appData.invoices.push({ id: getNextId('invoice'), number: refString, date: auditDate, vendor: auditVendorDept, totalAmount: 0, items: surplusItems, challans: [] });
            }
            if(shortageItems.length > 0) {
                appData.requisitions.push({ id: getNextId('requisition'), slipNo: refString, date: auditDate, department: auditVendorDept, items: shortageItems, type: 'Audit' });
            }

            // ডাবল এন্ট্রি ঠেকাতে শিট ক্লিয়ার করা
            appData.auditReport = []; 
            
            // গুগল শিটে নতুন স্টক সেভ করা
            if (typeof saveData === 'function') saveData(); 
            
            // সাথে সাথে ডিসপ্লে আপডেট করা 
            if (typeof renderAll === 'function') renderAll();
            try { 
                document.querySelector("button[onclick*=\"'itemList'\"]").click(); 
            } catch(e) {}
            ...
            if(resultDiv) {
                ...
                resultDiv.innerHTML = `<strong style="color:green; font-size:16px;">✅ সফলভাবে ${processedCount} টি আইটেমের স্টক ${formatDateToDDMMYYYY(auditDate)} তারিখে আপডেট হয়েছে!</strong>`;
            }
            
            alert(`✅ SUCCESS! Stock adjusted for ${processedCount} items.\n\nকী কী পরিবর্তন হয়েছে:\n${logText}`);
            
        } else {
            alert("⚠️ কোনো আইটেমের স্টক পরিবর্তন হয়নি। \nহয় আপনি গুগল শিটে কোনো নতুন সংখ্যা বসাননি, অথবা সংখ্যাগুলো সিস্টেমের স্টকের সাথে একদম মিলে গেছে।");
        }

    } catch (e) {
        console.error("Audit Sync Error: ", e);
        alert("❌ একটি টেকনিক্যাল এরর হয়েছে: " + e.message);
    }
}
</script>
